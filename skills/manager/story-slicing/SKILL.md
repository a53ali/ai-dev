...
## Agent-Native Jira Intelligence

You don't need Jira Premium to get AI-assisted issue management. These commands replicate the core Atlassian Intelligence features using the agent directly — paste a story, epic, or comment thread and get the output instantly.

---

### 🔀 Command: Work Breakdown (Epic → Child Stories)

> *"Break this epic into stories"* / *"Generate child issues from this epic"*

**Input:** Paste the epic title + description (or fetch via MCP: `jira_get_issue(issue_key: "PROJ-epic")`)

**What the agent does:**
1. Reads the epic and identifies the core user journeys and outcomes
2. Applies the 9 splitting patterns (see Step 3) to generate candidate stories
3. Checks each candidate against INVEST
4. Scans the codebase for complexity signals per slice (see Step 1)
5. Returns a numbered list of child stories, each with title, AC, size estimate, and splitting rationale

**Output format:**
```
Epic: <epic title>
Split strategy: <pattern used>

Child Stories:
1. [S] <title> — <one-line value statement>
   AC: Given…When…Then…
   Codebase impact: <services/endpoints/schema>
   Estimate: S (1–2 days)
...
### ✍️ Command: Story Enhancement

> *"Improve this ticket"* / *"Rewrite this story"* / *"This description is vague"*

**Input:** Paste the raw story text (title + description in any format)

**What the agent does:**
- Rewrites using the `As a / I want / So that` template
- Adds or completes missing acceptance criteria as Given/When/Then
- Flags INVEST violations and explains how to fix them
- Adjusts tone for the right audience (engineer vs. product vs. stakeholder)
- Ensures the story is specific enough to estimate but not over-specified
...
Acceptance Criteria:
- Given I type 3+ characters, When I submit, Then results matching the keyword appear
- Given no results match, Then I see "No results found" with suggested alternatives
- Given I clear the search, Then the full product list is restored

INVEST issues fixed:
- ❌ Not Estimable (original) → ✅ Added scope: keyword match on name/description only, no fuzzy match
- ❌ Not Testable (original) → ✅ AC added
```

---

### 💬 Command: Comment Thread Summary

> *"Summarize this ticket"* / *"What's the current status of this issue?"* / *"TLDR this thread"*

**Input:** Paste the comment thread, or fetch via MCP:
```
jira_get_issue(issue_key: "PROJ-123")   # includes description + comments
```
...
### Key context
- <important background or constraint>

### Pending / open questions
- <unresolved item 1> — needs input from <person/team>
...
### Recommendation
<one-paragraph summary of what needs to happen next>
```

---

### 🧪 Command: Edge Case Generator
...
**What the agent does:**
- Applies the happy-path/alternate-path split pattern systematically
- Generates error states, boundary conditions, and negative scenarios
- Groups by: validation errors, authorization edge cases, concurrency, empty states, performance limits
...
🔴 Error states:
  ...
  - What if the input exceeds max length?
  - What if the server returns 500?

🟡 Boundary conditions:
  - What if the user has exactly 0 items vs. 1 item?
  - What if the date is today vs. yesterday vs. tomorrow?

🟠 Authorization:
  - What if a non-owner tries to access this?
  - What if the session has expired mid-flow?

⚪ Empty/null states:
  - What does the UI show when there are no results?
  - What if a required field is null in the DB?

Suggested additional AC (ranked by risk):
1. [HIGH] Given the user submits with an empty field, Then validation error shown inline
2. [MEDIUM] Given the session expires during checkout, Then user is redirected to login with cart preserved
3. [LOW] Given 0 results match, Then "No results" message shown with a reset link
```

---

### 🔗 Command: Dependency Detector

> *"Find dependencies in these stories"* / *"Which stories are blocking each other?"*

**Input:** Paste a list of stories (titles + descriptions), or provide a Jira sprint/epic key for MCP fetch:
```
jira_search_issues(jql: "sprint in openSprints() AND project = PROJ")
```

**What the agent does:**
- Reads all stories and builds a dependency map
- Flags: shared data entities, shared API endpoints, sequential user flows, auth/permission preconditions
- Suggests a sequencing order to minimize blocking

**Output:**
```
Dependency Analysis — Sprint <N>

⛔ Hard dependencies (cannot start until blocker is done):
  PROJ-12 "User can checkout" → BLOCKED BY → PROJ-8 "User can add item to cart"
  PROJ-15 "Admin can view orders" → BLOCKED BY → PROJ-12 "User can checkout"

⚠️  Soft dependencies (can start in parallel but integration needed):
  PROJ-10 "Order confirmation email" — shares Order entity with PROJ-12
  PROJ-11 "Payment webhook handler" — depends on same Stripe integration as PROJ-9

✅ Fully independent (no blockers):
  PROJ-13 "User can view order history"
  PROJ-14 "User can cancel an order (happy path)"
...
### 🔢 Command: Story Point Estimator

> *"Estimate this story"* / *"How complex is this?"* / *"Give me a point estimate with reasoning"*

**Input:** Story title + description + codebase scan results (from Step 1)

**What the agent does:**
- Maps codebase complexity signals to a Fibonacci estimate (1, 2, 3, 5, 8, 13)
- Explains the reasoning in terms your team can debate
- Flags uncertainty (suggests a spike if confidence is low)

**Estimation model:**

| Signal | Points added |
|--------|-------------|
| 1 service touched, no schema change | Base: 1 |
| Each additional service | +1 |
| New DB table or migration | +2 |
| New external integration / API | +3 |
| Auth/permission change | +2 |
| >5 acceptance criteria | +1 |
| Team unfamiliarity with area | +2 |
...
Reasoning:
✅ 1 service touched (user-service) → base 1
✅ 1 new endpoint (POST /sessions) → +1
✅ Auth token generation (JWT) → +2
✅ Team hasn't touched auth module in 6 months → +2 (unfamiliarity)
→ Total: 6 → rounded to 5 (Fibonacci)

Confidence: Medium
If confidence is low: consider a 1-point spike on the JWT refresh flow first.
```

---

### 🔍 Command: Natural Language → JQL

> *"Find all stories stuck in progress for more than 5 days"*
> *"Show me unresolved bugs assigned to me this sprint"*
> *"Which epics have no child stories?"*

**What the agent does:** Translates your plain English into valid JQL — no syntax to memorize.

**Examples:**

| You say | JQL generated |
|---------|--------------|
...
| "Blockers created in the last 2 weeks" | `project = PROJ AND labels = blocked AND created >= -14d ORDER BY created DESC` |

If Jira MCP is available, the agent can execute the JQL directly:
```
...
### Fetch the epic or parent story from Jira:
```
jira_get_issue(issue_key: "PROJ-epic")
```

### Fetch all existing child stories (avoid duplication):
```
jira_search_issues(jql: "parent = PROJ-epic ORDER BY rank ASC")
```

### Search Confluence for the feature spec or PRD:
```
confluence_search(query: "<feature name> spec requirements", space_key: "PROD", limit: 5)
confluence_get_page(page_id: "<spec_page_id>")
```

### Create each slice as a child story in Jira:
```
jira_create_issue(
  project: "PROJ",
  issue_type: "Story",
  summary: "<slice title>",
  description: "<story template output>",
  parent: "PROJ-epic",
...
### Link a spike to the parent epic:
```
jira_create_issue(
  project: "PROJ",
  issue_type: "Spike",
  summary: "Spike: <investigation topic>",
  description: "Timebox: 2 days. Output: ADR + estimated stories.",
  parent: "PROJ-epic"
)
```

**Graceful fallback:** Output the full slice set as Markdown with the Jira story template — ready to copy-paste into tickets.
